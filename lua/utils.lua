...
function M.update_neovim()
  vim.notify("Updating NeoVim.", vim.log.levels.WARN, { title = "NyanVim" })
  if M.os == "Windows_NT" then
    vim.notify("Please check for NeoVim u.", vim.log.levels.WARN, { title = "NyanVim" })
    if vim.fn.has("nvim-0.7.0") == "0" then
      vim.notify(
        "Please update your NeoVim. You can probably update it with opening powershell session as addministrator and running choco install --force neovim. After that open NeoVim and run :NyanVimUpdate",
        vim.log.levels.WARN,
        { title = "NyanVim" }
      )
    end
  elseif M.os == "Linux" then
    ...
    vim.notify("NeoVim Updated.", vim.log.levels.WARN, { title = "NyanVim" })
  else
...
function M.update()
  ...
  -- Update NyanVim via git pull --ff-only
  ...
  if job_status == 0 then
    vim.api.nvim_notify("NyanVim Updated!", vim.log.levels.WARN, { title = "NyanVim" })
  else
    vim.notify("Update failed! Please try pulling manually.", vim.log.levels.ERROR, { title = "NyanVim" })
  end
end
